...
function validateFileUpload(req, res, next) {
    ...
    for (const file of files) {
        ...
        if (!allowedExtensions) {
            throw new ErrorResponse(400, "file_type_not_allowed", {mimetype: file.mimetype});
        }

        // Check file extension
        const fileExtension = path.extname(file.name).toLowerCase();
        if (!allowedExtensions.includes(fileExtension)) {
            throw new ErrorResponse(400, "file_extension_not_allowed", {extension: fileExtension, mimetype: file.mimetype});
        }

        // Check file name for security
        if (file.name.includes('..') || file.name.includes('/') || file.name.includes('\\')) {
            throw new ErrorResponse(400, "invalid_file_name");
        }

        // Check file size
        if (file.size > MAX_FILE_SIZE) {
            throw new ErrorResponse(400, "file_size_exceeds_limit", {limit: MAX_FILE_SIZE / (1024 * 1024)});
        }
    }
